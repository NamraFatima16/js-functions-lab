...
const p123 = new Product("A123", "Product 1", 8, 10, 10.0);
...
if (matches.length === 0)
  ...
  console.log("when we try to remove product by ID it will return true");
  ...
  console.log("when we try to removea product that doesnt exist it should return false");
  response =  cat.removeProductById("Id does not exist");
  //Expectation
  if(response === false){
    console.log('\tPassed');
  }
   ...
   //Expectation
   if(response.type === expectedResult.type && response.productIds[0] === expectedResult.productIds[0] ){
    console.log('\tPassed');
   }
   else{
    console.log('\tFailed');
